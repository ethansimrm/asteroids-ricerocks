...
missile_group = set([])
explosion_group = set([])
safety_margin = 10 # Rocks will spawn at least 10 pixels away from the ship
difficulty = 1 # As difficulty increases, rocks will be slightly faster for a given score 
#If difficulty is 0, rocks will not increase in speed as score increases
...
def group_group_collide(group1, group2):
    ...
    for sprite in set(group1):
        if group_collide(group2, sprite):
            group1.discard(sprite) #This will avoid returning an error if something collides with two things at once
            collision_counter += 1
...
def rock_spawner():
    ...
    if started:
        ...
        rock_vel = [(random.random() * .6 - .3) * difficulty * 0.1 * score , (random.random() * .6 - .3) * difficulty * 0.1 * score]
        rock_avel = random.random() * .2 - .1
        a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
        if len(rock_group) <= 12 and dist(rock_pos, my_ship.get_position()) > a_rock.get_radius() + my_ship.get_radius() + safety_margin:
